...
func _cmd_get_metrics(params: Dictionary) -> Dictionary:
	...
	result["data"]["object_resource_count"] = Performance.get_monitor(
		Performance.OBJECT_RESOURCE_COUNT
	)
	result["data"]["object_node_count"] = Performance.get_monitor(Performance.OBJECT_NODE_COUNT)
	result["data"]["object_orphan_node_count"] = Performance.get_monitor(
...
func _cmd_inject_key(params: Dictionary) -> Dictionary:
	...
	return {
		"type": "input_injected", "input_type": "key", "keycode": event.keycode, "pressed": pressed
	}
...
func _serialize_node_tree(
	node: Node, depth: int, max_depth: int, include_properties: bool
) -> Dictionary:
	...
	if value == null:
		...
		if value.has("_type"):
			match value["_type"]:
				...
				"Color":
					return Color(
						value.get("r", 0), value.get("g", 0), value.get("b", 0), value.get("a", 1)
					)
				"NodePath":
