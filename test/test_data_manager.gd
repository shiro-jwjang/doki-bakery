...
func test_balance_base_values():
	assert_eq(
		DataManager.balance.production.baseTimeMultiplier, 0.2, "Base time multiplier should be 0.2"
	)
	assert_eq(
		DataManager.balance.production.fairyLevelBonus, 0.05, "Fairy level bonus should be 0.05"
	)
	assert_eq(
		DataManager.balance.pricing.ingredientMultiplier, 2.5, "Ingredient multiplier should be 2.5"
	)
...
func test_balance_upgrade_configs():
	...
	assert_eq(
		DataManager.balance.upgrades.costMultiplier, 1.5, "Upgrade cost multiplier should be 1.5"
	)
...
func test_balance_offline_configs():
	...
	assert_eq(
		DataManager.balance.offline.rewardMultiplier, 0.5, "Offline reward multiplier should be 0.5"
	)
...
func test_balance_milestones():
	...
	assert_eq(
		DataManager.balance.milestones["5min"].target, 100, "5min milestone target should be 100"
	)
